...
public class TripMicroservice {
    ...
    private Booking initiateBooking(String name) {
        boolean flight = name.startsWith("flight-");
    ...
    private Booking getStatus(Booking booking) throws UnsupportedEncodingException {
        boolean flight = booking.getName().startsWith("flight-");
        WebTarget webTarget = (flight ? flightTarget : hotelTarget).path("/" + URLEncoder.encode(booking.getId(), "UTF-8"));
        Response responseData = webTarget.request().get();
        Booking responseBooking = responseData.readEntity(Booking.class);
